			...
			<?php if ($global_toggles['enable_comments']) : ?>
				<span class="dr-comment">
					...
					<a class="dr-comment-popup">
						...
						echo sprintf(esc_html(_nx('%s Comment', '%s Comments', number_format_i18n($recipe->comments_number), 'number of comments', 'delicious-recipes')), esc_html(number_format_i18n($recipe->comments_number)));
						?>
					</a>
				</span>
			<?php endif; ?>
...
<div class="dr-buttons">
	<?php if ($global_toggles['enable_jump_to_recipe']) : ?>
		<a href="#dr-recipe-meta-main-<?php echo esc_attr($recipe->ID); ?>" class="dr-btn-link dr-btn1 dr-smooth-scroll"><?php echo esc_html($global_toggles['jump_to_recipe_lbl']); ?> <svg class="icon">
	...
	<?php if (!empty($recipe->video_gallery) && $global_toggles['enable_jump_to_video']) : ?>
		<a href="#dr-video-gallery-<?php echo esc_attr($recipe->ID); ?>" class="dr-btn-link dr-btn1 dr-smooth-scroll"><?php echo esc_html($global_toggles['jump_to_video_lbl']); ?> <svg class="icon">
				<use xlink:href="<?php echo esc_url(plugin_dir_url(DELICIOUS_RECIPES_PLUGIN_FILE)); ?>assets/images/sprite.svg#play-button"></use>
			</svg></a>
	<?php endif; ?>
