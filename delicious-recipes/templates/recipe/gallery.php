...
<figure class="dr-feature-image">
	...
	if (has_post_thumbnail()) {
		the_post_thumbnail($img_size, ['class' => 'avoid-lazy-load']);
	} else {
		echo wp_get_attachment_image($recipe->thumbnail_id, $img_size, false, ['class' => 'avoid-lazy-load']);
	}
	?>
	...
	<?php if (isset( $recipe->enable_image_gallery ) && $recipe->enable_image_gallery && isset($recipe->image_gallery) && !empty($recipe->image_gallery)) : ?>
		...
		<a type="button" class="view-gallery-btn del-recipe-gallery-link" data-fslightbox="gallery" href="<?php echo esc_url($image['previewURL']); ?>">
		<?php endforeach; ?>
			<b><?php echo esc_html__("View Gallery", 'delicious-recipes'); ?></b>
			<span>
				<?php
				/* translators: %1$s: gallery images count */
				echo sprintf(esc_html(_nx('%1$s photo', '%1$s photos', count($recipe->image_gallery), 'gallery images count', 'delicious-recipes')), esc_html(number_format_i18n(count($recipe->image_gallery))));
				?>
			</span>
