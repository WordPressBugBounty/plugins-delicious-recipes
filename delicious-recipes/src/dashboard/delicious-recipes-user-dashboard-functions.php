...
function delicious_recipes_lostpassword_url() {
	...
	if ( ! did_action( 'init' ) ) {
		...
		if ( is_multisite() && isset( $_GET['redirect_to'] ) && false !== strpos( wp_sanitize_redirect(wp_unslash( $_GET['redirect_to'] )), network_admin_url() ) ) { // WPCS: input var ok, sanitization ok.
			$url = $default_url;
		} else {
