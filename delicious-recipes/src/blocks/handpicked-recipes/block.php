...
if ( ! function_exists( 'delicious_recipes_handpicked_recipes_block_render' ) ) {
	...
	function delicious_recipes_handpicked_recipes_block_render( $attributes ) {
		...
		echo '<div class="dr-recipes-handpicked-block ' . esc_attr( $class_name ) . '">';

		if ( $title ) {
			$allowed_headings = array( 'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'p' );
			$heading_tag      = in_array( $heading, $allowed_headings, true ) ? $heading : 'h2';
			printf( '<%1$s class="dr-entry-title">%2$s</%1$s>', esc_html( $heading_tag ), esc_html( $title ) );
		}
 ...
 */
if ( ! function_exists( 'delicious_recipes_handpicked_recipes_block_ajax' ) ) {
	/**
	 * Ajax from Backend for the block.
	 */
