...
class Delicious_Recipes_Recipe {
	...
	public function get_recipe( $recipe_id = false, $deprecated = array() ) {
		...
		$recipe_data = array(
			...
			'nutrition'            => array(
				...
				'servings'                      => $this->get_no_of_servings( $recipe_id ),
	...
	private function get_preparation_time_unit( $recipe_id ) {
		...
		$preparation_time_unit = delicious_recipes_get_time_units( $preparation_time, $preparation_time_unit );
	...
	private function get_cook_time_unit( $recipe_id ) {
		...
		$cook_time_unit = delicious_recipes_get_time_units( $cook_time, $cook_time_unit );
	...
	private function get_rest_time_unit( $recipe_id ) {
		...
		$rest_time_unit = delicious_recipes_get_time_units( $rest_time, $rest_time_unit );

		return apply_filters( 'wp_delicious_rest_time_unit', trim( $rest_time_unit ), $recipe_id );
	}
	...
	private function best_seasons() {
		...
		if ( ! empty( $custom_seasons ) ) {
			$seasons = array_merge( $seasons, $custom_seasons );
		}
	 ...
	 * Get recipe keywords.
	 *
	 * @param [type] $recipe_id Recipe ID.
	 */
